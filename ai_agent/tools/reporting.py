...
# ---------------------------------------------------------------------------
# Tier 2 bundle #7 - Professional Report Export (HTML / PDF + Roman Urdu)
# ---------------------------------------------------------------------------

def _roman_urdu_summary(target, rows, counts):
    """Generate the executive summary in Roman Urdu."""
    ...
    parts = []
    parts.append(
        "Jani, is report mein %s ka mukammal security assessment kiya gaya hai. "
        "Total %d finding(s) millin hain: %d critical, %d high, %d medium, "
        "%d low, %d info." % (target, len(rows), crit, high, med, low, info))
    if crit or high:
        parts.append(
            "Critical aur High severity ke issues sab se zyada khatarnak hain - "
            "in ko sab se pehle fix karna zaroori hai, kyunke in se system "
            "poori tarah compromise ho sakta hai.")
    top = [r for r in rows if r.get("severity") in ("critical", "high")][:3]
    if top:
        parts.append("Top khatarnak findings:")
        for i, r in enumerate(top, 1):
            parts.append(
                "%d) %s (%s) - %s" % (i, r.get("title", "untitled")[:80],
                                      r.get("severity", ""),
                                      (r.get("asset") or "")[:60]))
    parts.append(
        "Remediation: har finding ke saath fix ka tariqa diya gaya hai. "
        "Tamam issues close hone ke baad re-test karna must hai.")
    return "\n".join(parts)


def _render_html(target, author, now, rows, counts, roman_urdu_txt):
    """Build a self-contained professional HTML report."""
    sev_badge = {"critical": "#b91c1c", "high": "#dc2626",
                 "medium": "#d97706", "low": "#2563eb", "info": "#6b7280"}
    rows_html = []
    for i, r in enumerate(rows, 1):
        color = sev_badge.get(r.get("severity", "info"), "#6b7280")
        cwe = r.get("cwe") or ""
        evidence = (r.get("evidence") or "").strip()
        rem = (r.get("remediation") or "").strip()
        rows_html.append(
            '<div class="finding"><h3>%d. %s</h3>'
            '<p><span class="sev" style="background:%s">%s</span> '
            '<b>Asset:</b> %s &nbsp; <b>Status:</b> %s &nbsp; '
            '<b>CWE:</b> %s</p>'
            '<p class="desc">%s</p>'
            % (i, _html_esc(r.get("title") or "untitled"), color,
               _html_esc(r.get("severity") or "info"),
               _html_esc(r.get("asset") or "n/a"),
               _html_esc(r.get("status") or "open"), _html_esc(cwe),
               _html_esc((r.get("description") or "")[:400])))
        if evidence:
            rows_html.append('<pre>%s</pre>' % _html_esc(evidence[:1200]))
        if rem:
            rows_html.append('<p><b>Fix:</b> %s</p>' % _html_esc(rem[:600]))
        rows_html.append("</div>")
    counts_rows = "".join(
        "<tr><td>%s</td><td>%d</td></tr>" % (s.upper(), counts.get(s, 0))
        for s in SEVERITIES)
    ru = "<div class='ru'><h2>Roman Urdu Executive Summary</h2><p>%s</p></div>" \
         % _html_esc(roman_urdu_txt).replace("\n", "<br>")
    return ("<!DOCTYPE html><html><head><meta charset='utf-8'>"
            "<title>Pentest Report - %s</title><style>"
            ...
            ".meta{color:#475569}.finding{background:#f8fafc;border:1px solid "
            "#e2e8f0;border-radius:8px;padding:14px 18px;margin:14px 0}"
            ".sev{color:#fff;padding:2px 10px;border-radius:12px;font-size:12px}"
            ".desc{color:#334155}pre{background:#0f172a;color:#a5f3fc;padding:10px;"
            "border-radius:6px;overflow-x:auto}.ru{background:#fef3c7;border:1px "
            "solid #f59e0b;border-radius:8px;padding:14px 18px;margin:20px 0}"
            "table{border-collapse:collapse;margin:10px 0}"
            "td,th{border:1px solid #cbd5e1;padding:6px 14px}"
            "th{background:#e2e8f0}</style></head><body>"
            "<h1>Penetration Test Report</h1>"
            "<p class='meta'><b>Target:</b> %s<br><b>Date:</b> %s<br>"
            "<b>Author:</b> %s</p>"
            "<h2>1. Executive Summary</h2><p>This report documents the "
            "security assessment of %s. A total of %d finding(s) were "
            "identified.</p>"
            "<table><tr><th>Severity</th><th>Count</th></tr>%s</table>"
            "%s<h2>2. Findings</h2>%s"
            "<p class='meta'><i>Generated by HackerAI Agent - "
            "v14+ report export engine</i></p></body></html>"
            % (_html_esc(target), _html_esc(target), _html_esc(now),
               _html_esc(author), _html_esc(target), len(rows), counts_rows,
               ru, "".join(rows_html)))
...
# --- Minimal pure-stdlib PDF writer (zlib-compressed objects) ---------------
def _pdf_text_escape(t):
    return (str(t).replace(chr(92), chr(92) * 2).replace("(", chr(92) + "(")
            .replace(")", chr(92) + ")"))


def _pdf_wrap(text, width_chars=95):
...
def _write_pdf(path, title, meta_lines, rows):
    """Build a plain but valid PDF: title page + findings sections."""
    import zlib
    from io import BytesIO
    pages = []
    # page 1: title + meta + severity overview
    p1 = ["BT /F1 24 Tf 72 780 Td (PENETRATION TEST REPORT) Tj ET",
          "0.8 0.8 0.8 RG 72 760 468 1 re S"]
    y = 740
    for line in meta_lines:
        if y < 60:
            break
        p1.append("BT /F1 11 Tf 72 %d Td (%s) Tj ET" % (y, _pdf_text_escape(line)))
        y -= 18
    pages.append(p1)
    # findings pages
    page = ["BT /F1 16 Tf 72 780 Td (FINDINGS) Tj ET"]
    yy = 750
    for i, r in enumerate(rows, 1):
        head = "%d. [%s] %s  (asset: %s)" % (
            i, (r.get("severity") or "info").upper(),
            (r.get("title") or "untitled")[:90], (r.get("asset") or "n/a")[:40])
        lines = _pdf_wrap(head)
        lines += _pdf_wrap("CWE: %s | Status: %s"
                           % (r.get("cwe") or "-", r.get("status") or "open"))
        lines += _pdf_wrap("Desc: %s" % (r.get("description") or "")[:300])
        if r.get("evidence"):
            lines += _pdf_wrap("Evid: %s" % str(r["evidence"])[:300])
        if r.get("remediation"):
            lines += _pdf_wrap("Fix : %s" % str(r["remediation"])[:300])
        lines.append("")
        for ln in lines:
            if yy < 60:
                pages.append(page)
                page = ["BT /F1 14 Tf 72 780 Td (FINDINGS - continued) Tj ET"]
                yy = 750
            page.append("BT /F1 10 Tf 72 %d Td (%s) Tj ET"
                        % (yy, _pdf_text_escape(ln[:120])))
            yy -= 14
    pages.append(page)

    # PDF objects: 1=font 2=catalog 3=pages, then page/content pairs
    NLb = chr(10).encode("ascii")
    objects = [b"<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>"]
    objects.append(b"<< /Type /Catalog /Pages 3 0 R >>")
    kid_refs = b" ".join(b"%d 0 R" % (4 + i * 2) for i in range(len(pages)))
    objects.append(b"<< /Type /Pages /Kids [%s] /Count %d >>" % (kid_refs, len(pages)))
    for pi, content in enumerate(pages):
        stream = chr(10).join(content).encode("latin-1", errors="replace")
        cdata = zlib.compress(stream)
        objects.append(b"<< /Type /Page /Parent 3 0 R /MediaBox [0 0 612 792] "
                       b"/Resources << /Font << /F1 1 0 R >> >> "
                       b"/Contents %d 0 R >>" % (5 + pi * 2))
        objects.append(b"<< /Length %d /Filter /FlateDecode >>" % len(cdata)
                       + NLb + b"stream" + NLb + cdata + NLb + b"endstream")
    ...
    for i, obj in enumerate(objects, 1):
        ...
        buf.write(b"%d 0 obj\n" % i)
        data = obj if obj.endswith(b"stream\n") else obj
        buf.write(obj if obj.endswith(b"endstream") or b"\nstream" in obj
                  else obj + b"\n")
...
def tool_report_export(target="", author="HackerAI Agent", fmt="html",
                       roman_urdu=True, output_path="", include_open_only=True,
                       include_remediation=True):
    """Export the logged findings as a professional report.

    fmt           - 'html' (default) or 'pdf'
    roman_urdu    - include the Roman Urdu executive summary block
    output_path   - optional target file path
                    (default: reports/pentest_<ts>.<fmt>)
    include_open_only - only export open/confirmed/validated findings
    include_remediation - include remediation guidance per finding
    Returns the saved file path (and hit count) as JSON.
    """
    target = _clean_text(target, 300) or "Untitled Engagement"
    author = _clean_text(author, 100) or "HackerAI Agent"
    rows = _read_all()
    if not rows:
        return json.dumps({"error": "no findings logged yet - use "
                                    "add_finding first"}, ensure_ascii=False)
    filtered = [r for r in rows
                if r.get("status") == "false-positive"
                or r.get("verification") == "false-positive"]
    rows = [r for r in rows if r not in filtered]
    _apply_poc_cap(rows)  # in-place severity cap (PoC protocol)
    if include_open_only:
        keep = {"open", "confirmed", "validated",
                "needs-validation", "hypothesis"}
        rows = [r for r in rows if r.get("status") in keep]
    if not rows:
        return json.dumps({"error": "no findings match the filter"},
                          ensure_ascii=False)
    sev_order = {s: i for i, s in enumerate(SEVERITIES)}
    rows.sort(key=lambda r: sev_order.get(r.get("severity"), 99))
    counts = {s: 0 for s in SEVERITIES}
    for r in rows:
        counts[r.get("severity", "info")] = \
            counts.get(r.get("severity", "info"), 0) + 1
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    fmt = (fmt or "html").lower()
    if fmt not in ("html", "pdf"):
        return json.dumps({"error": "fmt must be 'html' or 'pdf'"},
                          ensure_ascii=False)
    ...
    if not output_path:
        ...
        output_path = os.path.join(os.path.dirname(__file__), "..", "..",
                                   "reports", "pentest_%s.%s" % (ts, ext))
    output_path = os.path.abspath(output_path)
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    ru_txt = _roman_urdu_summary(target, rows, counts) if roman_urdu else ""
    if fmt == "html":
        html = _render_html(target, author, now, rows, counts, ru_txt)
        ...
        try:
            ...
            story = [Paragraph("Penetration Test Report", styles["Title"]),
                     Paragraph("Target: %s" % target, styles["Normal"]),
                     Paragraph("Date: %s | Author: %s" % (now, author),
                               styles["Normal"]), Spacer(1, 12)]
            story.append(Paragraph("Executive Summary", styles["Heading1"]))
            story.append(Paragraph(
                "A total of %d finding(s) were identified." % len(rows),
                styles["Normal"]))
            
            if ru_txt:
                story.append(Paragraph("Roman Urdu Executive Summary",
                                       styles["Heading2"]))
                story.append(Paragraph(
                    ru_txt.replace("\n", "<br/>"), styles["Normal"]))
            ...
            for r in rows:
                story.append(Paragraph(
                    "<b>[%s] %s</b>" % (str(r.get("severity")).upper(),
                                        str(r.get("title") or "untitled")),
                    styles["Heading3"]))
                story.append(Paragraph(
                    "Asset: %s | CWE: %s | Status: %s"
                    % (r.get("asset") or "n/a", r.get("cwe") or "-",
                       r.get("status") or "open"), styles["Normal"]))
                if r.get("description"):
                    story.append(Paragraph(str(r["description"])[:800],
                                           styles["Normal"]))
                if include_remediation and r.get("remediation"):
                    story.append(Paragraph(
                        "<b>Fix:</b> " + str(r["remediation"])[:600],
                        styles["Normal"]))
            doc.build(story)
        except ImportError:
            _write_pdf(output_path, "Pentest Report",
                       ["Target: %s" % target, "Date: %s" % now,
                        "Author: %s" % author,
                        "Findings: %d" % len(rows)] +
                       (["", ru_txt] if ru_txt else []), rows)
    size = os.path.getsize(output_path)
    return json.dumps({"ok": True, "format": fmt, "path": output_path,
                       "findings": len(rows), "bytes": size,
                       "roman_urdu": bool(roman_urdu)},
