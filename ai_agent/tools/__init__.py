...
Tool("load_skill", "Load a full methodology guide for one skill into "
        ...
        Tool("attack_mission",
             ...
             {"type": "object",
              ...
              "required": ["target"]},
             lambda target="", phase="", campaign_dir="", run_nuclei="", budget_sec="": tool_attack_mission(
                 target, phase, campaign_dir,
                 str(run_nuclei).lower() in ("1", "true", "yes"),
                 int(budget_sec or 100))),
        Tool("mission_recon",
             "AUTO-PILOT phase 1: subdomain enum + fast TCP port scan + SSL info. Saves into campaigns/<target>.json.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir")},
              "required": ["target"]},
             lambda target="", campaign_dir="": tool_mission_recon(target, campaign_dir)),
        Tool("mission_scan",
             "AUTO-PILOT phase 2: HTTP fingerprint (status/tech/headers) on open web ports + shallow dir fuzz.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir")},
              "required": ["target"]},
             lambda target="", campaign_dir="": tool_mission_scan(target, campaign_dir)),
        Tool("mission_vuln",
             "AUTO-PILOT phase 3: CVE lookup per discovered tech + optional nuclei runs (run_nuclei='true'). Logs needs-validation findings.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir"),
        ...
        Tool("mission_report",
             ...
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir")},
              "required": ["target"]},
             lambda target="", campaign_dir="": tool_mission_report(target, campaign_dir)),
        Tool("mission_resume",
             "AUTO-PILOT: resume a mission from its campaign state - runs every unfinished phase.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir"),
                             "run_nuclei": _str_prop("true/false")},
              "required": ["target"]},
             lambda target="", campaign_dir="", run_nuclei="": tool_mission_resume(target, campaign_dir, str(run_nuclei).lower() in ("1", "true", "yes"))),
        Tool("mission_status",
             "AUTO-PILOT: show campaign progress for a target or list all campaigns.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain (empty = list all)"),
                             "campaign_dir": _str_prop("state dir")},
              "required": []},
             lambda target="", campaign_dir="": tool_mission_status(target, campaign_dir)),
        Tool("mission_reset",
             "AUTO-PILOT: delete a campaign state file to restart a mission clean.",
             {"type": "object",
              "properties": {"target": _str_prop("IP or domain"),
                             "campaign_dir": _str_prop("state dir")},
              "required": ["target"]},
